...
fill_leaf_prob(const bool VERBOSE,
  ...
  for (size_t i = 0; i < n_leaf; ++i) {
    ...
    if (VERBOSE) cerr << "Filled " << count << " missing sites in leaf"
                      << i << endl;
  }
...
trans_deriv(const vector<vector<double> > &Q,
  ...
  double term = kronecker_delta(cur, 1)*2 - 1.0 -
    time_trans_mat[anc][cur]*(G[prev][1]-G[prev][0])/denom;
  d_rate0 = T*G[prev][cur]/denom*term;

  if (prev == 0) {
    d_g1 = 0;
    term = kronecker_delta(cur, 0)*2 - 1.0 -
      G[prev][cur]*(time_trans_mat[anc][0] - time_trans_mat[anc][1])/denom ;
  ...
  term = Q[anc][cur] - time_trans_mat[anc][cur]*(G[prev][0]*Q[anc][0]+
                                                 G[prev][1]*Q[anc][1])/denom;
...
subtree_likelihood_site(const vector<size_t> &subtree_sizes,
  ...
  if (subtree_sizes[tree_start] == 1) {
    assert (tree_prob_table[pos][tree_start] > 0 && tree_prob_table[pos][tree_start] <1);
    subtree_lik_given_root_unmeth = tree_prob_table[pos][tree_start];
    subtree_lik_given_root_meth = 1.0 - tree_prob_table[pos][tree_start];
  }
  else {
    ...
    while (count < subtree_sizes[tree_start]) {
      ...
      double u, m;
      subtree_likelihood_site(subtree_sizes, rate, Ts,
                              child_start, pos, tree_prob_table, u, m);
      vector<vector<double> > child_trans_mat;
      temporal_trans_prob_mat(Ts[child_start], rate, child_trans_mat);
    ...
    tree_prob_table[pos][tree_start] = subtree_lik_given_root_unmeth/(subtree_lik_given_root_unmeth + subtree_lik_given_root_meth);
  }
}


static void
init_internal_prob_fel(const vector<size_t> &subtree_sizes,
                       const double rate0,
                       const vector<double> &Ts,
                       vector<vector<double> > &tree_prob_table){
  ...
  for (size_t pos = 0; pos < tree_prob_table.size(); ++ pos) {
    double u,m;
    subtree_likelihood_site(subtree_sizes, rate0, Ts,0,
                            pos, tree_prob_table, u, m);
  }
}
...
approx_posterior(const vector<size_t> &subtree_sizes,
  ...
  //initialize internal probs by felsenstein's recursion
  init_internal_prob_fel(subtree_sizes, rate0, Ts, meth_prob_table);
...
static void
approx_triad_weight(const vector<size_t> &subtree_sizes,
