      ...
      for (size_t i = 0; i < 2; ++i) {
        for (size_t j = 0; j < 2; ++j) {
          for (size_t k = 0; k < 2; ++k) {
            ...
            if (p_prev > 0.5 && p_cur > 0.5 && p_anc > 0.5)
              triad_weights[child_id][i][j][k] += 1.0;
          }
        }
      }
      count += subtree_sizes[child_id];
  ...
  while (count < subtree_sizes[node_id]) {
    ...
    for (size_t j = 0; j < 2; ++j) {
      for (size_t k = 0; k < 2; ++k) {
        get_posterior(tree_prob_table, pos, node_id, j, p_anc);
        get_posterior(tree_prob_table, pos, child_id, k, p_cur);
        if (p_cur > 0.5 && p_anc > 0.5)
          start_weights[child_id][j][k] += 1.0;
      }
    }
    count += subtree_sizes[child_id];
  }
        ...
        while (count < subtree_sizes[node_id]) {
          ...
          for (size_t j = 0; j < 2; ++j) {
            for (size_t k = 0; k < 2; ++k) {
              ...
              get_posterior(tree_prob_table, pos, node_id, j, p_anc);
              get_posterior(tree_prob_table, pos, child_id, k, p_cur);
              if (p_anc > 0.5 && p_cur > 0.5)
                inherit_weights[child_id][j][k] += 1.0;
            }
          }
          count += subtree_sizes[child_id];
        }
          ...
          if (subtree_sizes[child_id] > 1) {
            ...
            for (size_t j = 0; j < 2; ++j) {
              for (size_t x = 0; x < 2; ++x) {
                ...
                if (p_anc > 0.5 && p_l_gnd > 0.5)
                  inherit_left_weights[child_id][j][x] += 1.0;
                if (p_anc > 0.5 && p_r_gnd > 0.5)
                  inherit_right_weights[child_id][j][x] += 1.0;
              }
            }
          }
          count += subtree_sizes[child_id];
...
get_autocor_weights(const vector<size_t> &subtree_sizes,
  ...
  for (size_t block = 0; block < reset_points.size()-1; ++ block){
    size_t start = reset_points[block];
    size_t end = reset_points[block+1];
    for (size_t pos = start; pos < end -1; ++pos) {
      for (size_t node_id = 0; node_id < subtree_sizes.size(); ++ node_id) {
        for (size_t i = 0; i < 2; ++i) {
          for (size_t k = 0; k < 2; ++k) {
            double p_cur, p_prev;
            get_posterior(tree_prob_table, pos, node_id, i, p_prev);
            get_posterior(tree_prob_table, pos+1, node_id, k, p_cur);
            if (p_prev > 0.5 && p_cur > 0.5)
              autocor_weights[i][k] += 1.0;
          }
        }
      }
    }
...
objective_branch(const vector<size_t> &subtree_sizes,
  ...
  for (size_t j = 0; j < 2; ++j) {
    for (size_t k = 0; k < 2; ++k) {
      F += inherit_weights[node_id][j][k]*log(PT[j][k]);
...
update_G(const double TOL,
         const vector<vector<double> > &autocor_weights,
         double &new_g0, double &new_g1) {
  new_g0 = autocor_weights[0][0]/(autocor_weights[0][0]+autocor_weights[0][1]);
  new_g1 = autocor_weights[1][1]/(autocor_weights[1][1]+autocor_weights[1][0]);
  new_g0 = min(max(TOL, new_g0), 1-TOL);
  new_g1 = min(max(TOL, new_g1), 1-TOL);
}
...
optimize_params(const vector<size_t> &subtree_sizes,
  ...
  double new_g0;
  double new_g1;
  update_G(TOL, autocor_weights, new_g0, new_g1);
  newparams[2] = new_g0;
  newparams[3] = new_g1;
